# js笔试题记录

## 继承

[创建对象之工厂模式](./class-extend-methods/_1factory-pattern.md)

[创建对象之构造函数模式](./class-extend-methods/_2constructor-pattern.md)

[创建对象之原型模式](./class-extend-methods/_3prototype-pattern.md)

[创建对象之组合模式](./class-extend-methods/_4constructor_prototype.md)

[new 操作符，__proto__，prototype](./class-extend-methods/_6new-operator.md)

## 正则

[正则解析一个完整的url](./regexp-url/note.md)

[给数字添加千分符](./format-number/note.md)


## 功能函数
[防抖](./debounce/note.md)

[节流](./throttle/note.md)

[链式调用——实现add(1)(2)(3)...(n)](./chained-calls/note.md)

## 函数式编程

[柯里化](./currying/note.md)

[纯函数](./pure-functions/note.md)

## 未分类

[详解Object.create(null)](./Object_create/note.md)

[ES6 class & class extends & rest arguments](./syntactic-sugar/note.md)

[同步渲染和异步渲染对 DOM 的阻塞](./async-render/node.md)

## 作用域相关问题

[作用域相关问题](./scope-related/note.md)
